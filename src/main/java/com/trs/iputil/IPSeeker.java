...
public class IPSeeker {
    ...
    //纯真IP数据库名
	private String IP_FILE= this.getClass().getResource("/").getPath()+"qqwry.dat";
	
	//34个省
	private static final String[] areaArray = new String[]{"上海市","北京市","浙江省","广东省","天津市","重庆市","黑龙江","吉林省","辽宁省","内蒙古","河北省","新疆","甘肃省","青海省","陕西省","宁夏","河南省","山东省","山西省","安徽省","湖北省","湖南省","江苏省","四川省","贵州省","云南省","广西省","西藏","江西省","福建省","台湾省","海南省"};
	// 一些固定常量，比如记录长度等等
		...
		try {
			...
			if(b == REDIRECT_MODE_1) {
				...
				if(b == REDIRECT_MODE_2) {
					areaFlag = readString(readLong3());
					if(isChinaArea(areaFlag)){
						loc.setCountry ("中国");
						loc.setArea(areaFlag);
						ipFile.seek(countryOffset + 4);
					}else{
						loc.setCountry (areaFlag);
						ipFile.seek(countryOffset + 4);
						// 读取地区标志
					...
					areaFlag = readString(countryOffset);
					if(isChinaArea(areaFlag)){
						loc.setCountry ("中国");
						loc.setArea(areaFlag);
					}else{
				...
				areaFlag = readString(ipFile.getFilePointer() - 1);
				if(isChinaArea(areaFlag)){
					loc.setCountry ("中国");
					loc.setArea(areaFlag);
				}else{
	 ...
	 * @return 地区名字符串
	 */
	private boolean isChinaArea(String area) {
		for (int i = 0; i < areaArray.length; i++) {
			if(area.indexOf(areaArray[i])!= -1){
				return true;
			}
		}
		return false;
